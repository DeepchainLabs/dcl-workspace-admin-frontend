...
export default function OrganizationUserStats(){
    ...
    return (
        ...
        {organizationStats && organizationStats?.map((stats: any, index: number) => (
            <div key={index}><StatsCard number={stats.count} bgColor={"#F0F5FF"} textColor={"#2377FC"} title={stats.category || "N/A"} svg={<UserIcon personal={false}></UserIcon>} small={true}></StatsCard></div>
        ))}
        </div>
        </>
    )
